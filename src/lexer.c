...
void push(TokenList* node, TokenType type, char *lexeme) {
    ...
    node->next = (TokenList*)malloc(sizeof(TokenList));
    node->next->type = type;
    node->next->lexeme = lexeme;
    node->next->next = NULL;
}

void display(TokenList* node) {
    while (node->next != NULL) {
        printf("TYPE: %s\t", tk_map[node->type]);
        printf("LEXEME: %s\n", node->lexeme);
        node = node->next;
    }
}
...
TokenList* startLexer(const char* contents, unsigned int* return_error) {
    ...
    TokenList* tokenlist = initTokenList(tok->type, tok->lexeme);
    while (tok->type != TK_EOF) {
        ...
        push(tokenlist, tok->type, tok->lexeme);
    }
