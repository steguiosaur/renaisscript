    ...
    while (lexer->c != '\0' && lexer->i < strlen(lexer->contents)) {
        ...
        switch (lexer->c) {
            ...
            case ')':
                ...
                break;
            case '{':
                return lexer_advance_with_token(lexer, init_token(TOKEN_LCURLY, lexer_get_current_char_as_string(lexer)));
                break;
            case '}':
                return lexer_advance_with_token(lexer, init_token(TOKEN_RCURLY, lexer_get_current_char_as_string(lexer)));
                break;
        }
    }
    return (void*)0;
